...
@Component
@Slf4j
public class EETimesCrawler extends CrawlerBase {

    private static final String RSS_URL = "https://www.eetimes.com/feed/";

    public EETimesCrawler(TrendItemRepository trendItemRepository) {
        super(trendItemRepository);
    }

    @Override
    public CrawlSource getCrawlSource() {
        return CrawlSource.EE_TIMES;
    }

    @Override
    public List<TrendItem> crawl() {
        List<TrendItem> results = new ArrayList<>();
        log.info("Starting EE Times RSS crawl");
        ...
        try {
            ...
            for (Element item : items) {
                ...
                TrendItem trendItem = TrendItem.builder()
                    ...
                    .originalUrl(url)
                    .urlHash(urlHash)
                    .source(TrendItem.CrawlSource.EE_TIMES)
            ...
            log.error("EE Times crawl failed: {}", e.getMessage());
        }

        log.info("EE Times crawl complete: {} new items", results.size());
        return results;
    }
}
