...
@Component
@Slf4j
public class ZigbangCrawler extends CrawlerBase {

    private static final String RSS_URL = "https://medium.com/feed/zigbang";

    public ZigbangCrawler(TrendItemRepository trendItemRepository) {
        super(trendItemRepository);
    }

    @Override
    public CrawlSource getCrawlSource() {
        return CrawlSource.ZIGBANG;
    }

    @Override
    public List<TrendItem> crawl() {
        List<TrendItem> results = new ArrayList<>();
        log.info("Starting Zigbang RSS crawl");
        try {
            ...
            for (Element item : items) {
                ...
                TrendItem trendItem = TrendItem.builder()
                    ...
                    .source(CrawlSource.ZIGBANG).description(description)
            ...
            log.error("Zigbang crawl failed: {}", e.getMessage());
        }
        log.info("Zigbang crawl complete: {} new items", results.size());
        return results;
    }
}
