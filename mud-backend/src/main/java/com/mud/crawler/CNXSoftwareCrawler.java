...
@Component
@Slf4j
public class CNXSoftwareCrawler extends CrawlerBase {

    private static final String RSS_URL = "https://www.cnx-software.com/feed/";

    public CNXSoftwareCrawler(TrendItemRepository trendItemRepository) {
        super(trendItemRepository);
    }

    @Override
    public CrawlSource getCrawlSource() {
        return CrawlSource.CNX_SOFTWARE;
    }

    @Override
    public List<TrendItem> crawl() {
        List<TrendItem> results = new ArrayList<>();
        log.info("Starting CNX Software RSS crawl");
        ...
        try {
            ...
            for (Element item : items) {
                ...
                TrendItem trendItem = TrendItem.builder()
                    ...
                    .originalUrl(url)
                    .urlHash(urlHash)
                    .source(TrendItem.CrawlSource.CNX_SOFTWARE)
            ...
            log.error("CNX Software crawl failed: {}", e.getMessage());
        }

        log.info("CNX Software crawl complete: {} new items", results.size());
        return results;
    }
}
