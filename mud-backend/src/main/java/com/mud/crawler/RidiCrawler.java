...
@Component
@Slf4j
public class RidiCrawler extends CrawlerBase {

    private static final String RSS_URL = "https://www.ridicorp.com/feed";

    public RidiCrawler(TrendItemRepository trendItemRepository) {
        super(trendItemRepository);
    }

    @Override
    public CrawlSource getCrawlSource() {
        return CrawlSource.RIDI;
    }

    @Override
    public List<TrendItem> crawl() {
        List<TrendItem> results = new ArrayList<>();
        log.info("Starting Ridi RSS crawl");
        try {
            ...
            for (Element item : items) {
                ...
                TrendItem trendItem = TrendItem.builder()
                    ...
                    .source(CrawlSource.RIDI).description(description)
            ...
            log.error("Ridi crawl failed: {}", e.getMessage());
        }
        log.info("Ridi crawl complete: {} new items", results.size());
        return results;
    }
}
