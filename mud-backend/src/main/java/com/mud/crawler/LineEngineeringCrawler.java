...
@Component
@Slf4j
public class LineEngineeringCrawler extends CrawlerBase {

    private static final String RSS_URL = "https://techblog.lycorp.co.jp/ko/feed/index.xml";

    public LineEngineeringCrawler(TrendItemRepository trendItemRepository) {
        super(trendItemRepository);
    }

    @Override
    public CrawlSource getCrawlSource() {
        return CrawlSource.LINE_ENGINEERING;
    }

    @Override
    public List<TrendItem> crawl() {
        List<TrendItem> results = new ArrayList<>();
        log.info("Starting LINE Engineering RSS crawl");
        ...
        try {
            ...
            for (Element item : items) {
                ...
                TrendItem trendItem = TrendItem.builder()
                    ...
                    .originalUrl(url)
                    .urlHash(urlHash)
                    .source(CrawlSource.LINE_ENGINEERING)
            ...
            log.error("LINE Engineering crawl failed: {}", e.getMessage());
        }

        log.info("LINE Engineering crawl complete: {} new items", results.size());
        return results;
    }
}
