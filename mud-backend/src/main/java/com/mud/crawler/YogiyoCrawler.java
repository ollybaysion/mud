...
@Component
@Slf4j
public class YogiyoCrawler extends CrawlerBase {

    private static final String RSS_URL = "https://techblog.yogiyo.co.kr/feed";

    public YogiyoCrawler(TrendItemRepository trendItemRepository) {
        super(trendItemRepository);
    }

    @Override
    public CrawlSource getCrawlSource() {
        return CrawlSource.YOGIYO;
    }

    @Override
    public List<TrendItem> crawl() {
        List<TrendItem> results = new ArrayList<>();
        log.info("Starting Yogiyo RSS crawl");
        try {
            ...
            for (Element item : items) {
                ...
                TrendItem trendItem = TrendItem.builder()
                    ...
                    .source(CrawlSource.YOGIYO).description(description)
            ...
            log.error("Yogiyo crawl failed: {}", e.getMessage());
        }
        log.info("Yogiyo crawl complete: {} new items", results.size());
        return results;
    }
}
