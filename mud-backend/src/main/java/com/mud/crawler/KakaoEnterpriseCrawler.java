...
@Component
@Slf4j
public class KakaoEnterpriseCrawler extends CrawlerBase {

    private static final String RSS_URL = "https://tech.kakaoenterprise.com/feed";

    public KakaoEnterpriseCrawler(TrendItemRepository trendItemRepository) {
        super(trendItemRepository);
    }

    @Override
    public CrawlSource getCrawlSource() {
        return CrawlSource.KAKAO_ENTERPRISE;
    }

    @Override
    public List<TrendItem> crawl() {
        List<TrendItem> results = new ArrayList<>();
        log.info("Starting Kakao Enterprise RSS crawl");
        try {
            ...
            for (Element item : items) {
                ...
                TrendItem trendItem = TrendItem.builder()
                    .title(title).originalUrl(url).urlHash(urlHash)
                    .source(CrawlSource.KAKAO_ENTERPRISE).description(description)
                    .publishedAt(publishedAt).crawledAt(LocalDateTime.now())
                    .analysisStatus(TrendItem.AnalysisStatus.PENDING).build();
                TrendItem saved = saveIfNew(trendItem);
                if (saved != null) results.add(saved);
            }
        } catch (Exception e) {
            log.error("Kakao Enterprise crawl failed: {}", e.getMessage());
        }
        log.info("Kakao Enterprise crawl complete: {} new items", results.size());
        return results;
    }
}
