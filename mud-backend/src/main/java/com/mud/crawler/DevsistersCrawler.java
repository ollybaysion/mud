...
@Component
@Slf4j
public class DevsistersCrawler extends CrawlerBase {

    private static final String RSS_URL = "https://tech.devsisters.com/rss.xml";

    public DevsistersCrawler(TrendItemRepository trendItemRepository) {
        super(trendItemRepository);
    }

    @Override
    public CrawlSource getCrawlSource() {
        return CrawlSource.DEVSISTERS;
    }

    @Override
    public List<TrendItem> crawl() {
        List<TrendItem> results = new ArrayList<>();
        log.info("Starting Devsisters RSS crawl");
        try {
            ...
            for (Element item : items) {
                ...
                TrendItem trendItem = TrendItem.builder()
                    ...
                    .source(CrawlSource.DEVSISTERS).description(description)
            ...
            log.error("Devsisters crawl failed: {}", e.getMessage());
        }
        log.info("Devsisters crawl complete: {} new items", results.size());
        return results;
    }
}
