...
@Component
@Slf4j
public class PhoronixCrawler extends CrawlerBase {

    private static final String RSS_URL = "https://www.phoronix.com/rss.php";

    public PhoronixCrawler(TrendItemRepository trendItemRepository) {
        super(trendItemRepository);
    }

    @Override
    public CrawlSource getCrawlSource() {
        return CrawlSource.PHORONIX;
    }

    @Override
    public List<TrendItem> crawl() {
        List<TrendItem> results = new ArrayList<>();
        log.info("Starting Phoronix RSS crawl");
        ...
        try {
            ...
            log.error("Phoronix crawl failed: {}", e.getMessage());
        }

        log.info("Phoronix crawl complete: {} new items", results.size());
        return results;
    }
}
