...
@Component
@Slf4j
public class BanksaladCrawler extends CrawlerBase {

    private static final String RSS_URL = "https://blog.banksalad.com/rss.xml";

    public BanksaladCrawler(TrendItemRepository trendItemRepository) {
        super(trendItemRepository);
    }

    @Override
    public CrawlSource getCrawlSource() {
        return CrawlSource.BANKSALAD;
    }

    @Override
    public List<TrendItem> crawl() {
        List<TrendItem> results = new ArrayList<>();
        log.info("Starting Banksalad RSS crawl");
        try {
            ...
            for (Element item : items) {
                ...
                TrendItem trendItem = TrendItem.builder()
                    ...
                    .source(CrawlSource.BANKSALAD).description(description)
            ...
            log.error("Banksalad crawl failed: {}", e.getMessage());
        }
        log.info("Banksalad crawl complete: {} new items", results.size());
        return results;
    }
}
