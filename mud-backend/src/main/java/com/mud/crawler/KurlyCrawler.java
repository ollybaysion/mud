...
@Component
@Slf4j
public class KurlyCrawler extends CrawlerBase {

    private static final String RSS_URL = "https://helloworld.kurly.com/feed.xml";

    public KurlyCrawler(TrendItemRepository trendItemRepository) {
        super(trendItemRepository);
    }

    @Override
    public CrawlSource getCrawlSource() {
        return CrawlSource.KURLY;
    }

    @Override
    public List<TrendItem> crawl() {
        List<TrendItem> results = new ArrayList<>();
        log.info("Starting Kurly RSS crawl");
        try {
            ...
            for (Element item : items) {
                ...
                TrendItem trendItem = TrendItem.builder()
                    ...
                    .source(CrawlSource.KURLY).description(description)
            ...
            log.error("Kurly crawl failed: {}", e.getMessage());
        }
        log.info("Kurly crawl complete: {} new items", results.size());
        return results;
    }
}
