...
@Component
@Slf4j
public class CoupangEngineeringCrawler extends CrawlerBase {

    private static final String RSS_URL = "https://medium.com/feed/coupang-engineering";

    public CoupangEngineeringCrawler(TrendItemRepository trendItemRepository) {
        super(trendItemRepository);
    }

    @Override
    public CrawlSource getCrawlSource() {
        return CrawlSource.COUPANG_ENGINEERING;
    }

    @Override
    public List<TrendItem> crawl() {
        List<TrendItem> results = new ArrayList<>();
        log.info("Starting Coupang Engineering RSS crawl");
        ...
        try {
            ...
            for (Element item : items) {
                ...
                TrendItem trendItem = TrendItem.builder()
                    ...
                    .originalUrl(url)
                    .urlHash(urlHash)
                    .source(CrawlSource.COUPANG_ENGINEERING)
            ...
            log.error("Coupang Engineering crawl failed: {}", e.getMessage());
        }

        log.info("Coupang Engineering crawl complete: {} new items", results.size());
        return results;
    }
}
