...
@Component
@Slf4j
public class SocarCrawler extends CrawlerBase {

    private static final String RSS_URL = "https://tech.socarcorp.kr/feed";

    public SocarCrawler(TrendItemRepository trendItemRepository) {
        super(trendItemRepository);
    }

    @Override
    public CrawlSource getCrawlSource() {
        return CrawlSource.SOCAR;
    }

    @Override
    public List<TrendItem> crawl() {
        List<TrendItem> results = new ArrayList<>();
        log.info("Starting Socar RSS crawl");
        try {
            ...
            for (Element item : items) {
                ...
                TrendItem trendItem = TrendItem.builder()
                    ...
                    .source(CrawlSource.SOCAR).description(description)
            ...
            log.error("Socar crawl failed: {}", e.getMessage());
        }
        log.info("Socar crawl complete: {} new items", results.size());
        return results;
    }
}
