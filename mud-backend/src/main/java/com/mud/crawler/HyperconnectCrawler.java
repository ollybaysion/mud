...
@Component
@Slf4j
public class HyperconnectCrawler extends CrawlerBase {

    private static final String RSS_URL = "https://hyperconnect.github.io/feed.xml";

    public HyperconnectCrawler(TrendItemRepository trendItemRepository) {
        super(trendItemRepository);
    }

    @Override
    public CrawlSource getCrawlSource() {
        return CrawlSource.HYPERCONNECT;
    }

    @Override
    public List<TrendItem> crawl() {
        List<TrendItem> results = new ArrayList<>();
        log.info("Starting Hyperconnect RSS crawl");
        try {
            ...
            for (Element item : items) {
                ...
                TrendItem trendItem = TrendItem.builder()
                    ...
                    .source(CrawlSource.HYPERCONNECT).description(description)
            ...
            log.error("Hyperconnect crawl failed: {}", e.getMessage());
        }
        log.info("Hyperconnect crawl complete: {} new items", results.size());
        return results;
    }
}
