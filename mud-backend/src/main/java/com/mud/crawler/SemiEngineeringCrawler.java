...
@Component
@Slf4j
public class SemiEngineeringCrawler extends CrawlerBase {

    private static final String RSS_URL = "https://semiengineering.com/feed/";

    public SemiEngineeringCrawler(TrendItemRepository trendItemRepository) {
        super(trendItemRepository);
    }

    @Override
    public CrawlSource getCrawlSource() {
        return CrawlSource.SEMI_ENGINEERING;
    }

    @Override
    public List<TrendItem> crawl() {
        List<TrendItem> results = new ArrayList<>();
        log.info("Starting Semiconductor Engineering RSS crawl");
        ...
        try {
            ...
            for (Element item : items) {
                ...
                TrendItem trendItem = TrendItem.builder()
                    ...
                    .originalUrl(url)
                    .urlHash(urlHash)
                    .source(TrendItem.CrawlSource.SEMI_ENGINEERING)
            ...
            log.error("Semiconductor Engineering crawl failed: {}", e.getMessage());
        }

        log.info("Semiconductor Engineering crawl complete: {} new items", results.size());
        return results;
    }
}
