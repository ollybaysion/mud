...
@Component
@Slf4j
public class TheNewStackCrawler extends CrawlerBase {

    private static final String RSS_URL = "https://thenewstack.io/feed/";

    public TheNewStackCrawler(TrendItemRepository trendItemRepository) {
        super(trendItemRepository);
    }

    @Override
    public CrawlSource getCrawlSource() {
        return CrawlSource.THE_NEW_STACK;
    }

    @Override
    public List<TrendItem> crawl() {
        List<TrendItem> results = new ArrayList<>();
        log.info("Starting The New Stack crawl");
        ...
        try {
            ...
            for (Element item : items) {
                ...
                TrendItem trendItem = TrendItem.builder()
                    ...
                    .originalUrl(url)
                    .urlHash(urlHash)
                    .source(TrendItem.CrawlSource.THE_NEW_STACK)
                ...
                if (saved != null) {
                    results.add(saved);
                    log.debug("Saved The New Stack: {}", title);
                }
            }

            log.info("The New Stack crawl complete: {} new items", results.size());
        } catch (Exception e) {
            log.error("The New Stack crawl failed", e);
        }
