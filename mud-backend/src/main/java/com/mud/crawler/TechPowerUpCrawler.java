...
@Component
@Slf4j
public class TechPowerUpCrawler extends CrawlerBase {

    private static final String RSS_URL = "https://www.techpowerup.com/rss/news";

    public TechPowerUpCrawler(TrendItemRepository trendItemRepository) {
        super(trendItemRepository);
    }

    @Override
    public CrawlSource getCrawlSource() {
        return CrawlSource.TECHPOWERUP;
    }

    @Override
    public List<TrendItem> crawl() {
        List<TrendItem> results = new ArrayList<>();
        log.info("Starting TechPowerUp RSS crawl");
        ...
        try {
            ...
            for (Element item : items) {
                ...
                TrendItem trendItem = TrendItem.builder()
                    ...
                    .originalUrl(url)
                    .urlHash(urlHash)
                    .source(TrendItem.CrawlSource.TECHPOWERUP)
            ...
            log.error("TechPowerUp crawl failed: {}", e.getMessage());
        }

        log.info("TechPowerUp crawl complete: {} new items", results.size());
        return results;
    }
}
