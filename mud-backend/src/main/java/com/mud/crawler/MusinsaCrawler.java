...
@Component
@Slf4j
public class MusinsaCrawler extends CrawlerBase {

    private static final String RSS_URL = "https://medium.com/feed/musinsa-tech";

    public MusinsaCrawler(TrendItemRepository trendItemRepository) {
        super(trendItemRepository);
    }

    @Override
    public CrawlSource getCrawlSource() {
        return CrawlSource.MUSINSA;
    }

    @Override
    public List<TrendItem> crawl() {
        List<TrendItem> results = new ArrayList<>();
        log.info("Starting Musinsa RSS crawl");
        try {
            ...
            for (Element item : items) {
                ...
                TrendItem trendItem = TrendItem.builder()
                    ...
                    .source(CrawlSource.MUSINSA).description(description)
        ...
        log.info("Musinsa crawl complete: {} new items", results.size());
        return results;
    }
}
