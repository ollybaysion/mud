...
@Component
@Slf4j
public class NvidiaBlogCrawler extends CrawlerBase {

    private static final String RSS_URL = "https://blogs.nvidia.com/feed/";

    public NvidiaBlogCrawler(TrendItemRepository trendItemRepository) {
        super(trendItemRepository);
    }

    @Override
    public CrawlSource getCrawlSource() {
        return CrawlSource.NVIDIA_BLOG;
    }

    @Override
    public List<TrendItem> crawl() {
        List<TrendItem> results = new ArrayList<>();
        log.info("Starting NVIDIA Blog RSS crawl");
        ...
        try {
            ...
            for (Element item : items) {
                ...
                TrendItem trendItem = TrendItem.builder()
                    ...
                    .originalUrl(url)
                    .urlHash(urlHash)
                    .source(TrendItem.CrawlSource.NVIDIA_BLOG)
                ...
                if (saved != null) {
                    results.add(saved);
                    log.debug("Saved NVIDIA Blog item: {}", title);
                }
            }
        } catch (Exception e) {
            log.error("NVIDIA Blog crawl failed: {}", e.getMessage());
        }

        log.info("NVIDIA Blog crawl complete: {} new items", results.size());
        return results;
    }
}
