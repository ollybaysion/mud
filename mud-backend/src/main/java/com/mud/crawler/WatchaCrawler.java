...
@Component
@Slf4j
public class WatchaCrawler extends CrawlerBase {

    private static final String RSS_URL = "https://medium.com/feed/watcha";

    public WatchaCrawler(TrendItemRepository trendItemRepository) {
        super(trendItemRepository);
    }

    @Override
    public CrawlSource getCrawlSource() {
        return CrawlSource.WATCHA;
    }

    @Override
    public List<TrendItem> crawl() {
        List<TrendItem> results = new ArrayList<>();
        log.info("Starting Watcha RSS crawl");
        try {
            ...
            for (Element item : items) {
                ...
                TrendItem trendItem = TrendItem.builder()
                    ...
                    .source(CrawlSource.WATCHA).description(description)
            ...
            log.error("Watcha crawl failed: {}", e.getMessage());
        }
        log.info("Watcha crawl complete: {} new items", results.size());
        return results;
    }
}
