...
@Component
@Slf4j
public class FortyFourBitsCrawler extends CrawlerBase {

    private static final String RSS_URL = "https://www.44bits.io/ko/feed/all";

    public FortyFourBitsCrawler(TrendItemRepository trendItemRepository) {
        super(trendItemRepository);
    }

    @Override
    public CrawlSource getCrawlSource() {
        return CrawlSource.FORTYFOUR_BITS;
    }

    @Override
    public List<TrendItem> crawl() {
        List<TrendItem> results = new ArrayList<>();
        log.info("Starting 44bits RSS crawl");
        try {
            ...
            for (Element item : items) {
                ...
                TrendItem trendItem = TrendItem.builder()
                    ...
                    .source(CrawlSource.FORTYFOUR_BITS).description(description)
            ...
            log.error("44bits crawl failed: {}", e.getMessage());
        }
        log.info("44bits crawl complete: {} new items", results.size());
        return results;
    }
}
