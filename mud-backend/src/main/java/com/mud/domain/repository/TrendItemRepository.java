...
public interface TrendItemRepository extends JpaRepository<TrendItem, Long> {
    ...
    Page<TrendItem> findByAnalysisStatusInOrderByCrawledAtAsc(List<TrendItem.AnalysisStatus> statuses, Pageable pageable);
    ...
    List<TrendItem> findByAnalysisStatusAndCrawledAtBetweenAndScoreTotalIsNull(
        TrendItem.AnalysisStatus status, java.time.LocalDateTime from, java.time.LocalDateTime to
    );
}
